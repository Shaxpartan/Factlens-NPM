...
test('6.1.0 release metadata and runtime SDK header stay synchronized', () => {
  ...
  assert.match(httpSource, /SDK_VERSION\s*=\s*["']6\.1\.0["']/);
});

test('6.1.0 keeps image claims optional in CLI and SDK', () => {
  ...
  assert.match(changelog, /## 6\.1\.0/);
});

test('6.1.0 keeps multi result rendering and request in progress recovery', () => {
...
test('6.1.0 preserves existing media, progress, and source preference documentation', () => {
