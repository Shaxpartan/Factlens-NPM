...
test('6.5.0 release metadata and runtime SDK header stay synchronized', () => {
  assert.equal(packageJson.version, '6.5.0');
  assert.equal(packageLock.version, '6.5.0');
  assert.equal(packageLock.packages?.['']?.version, '6.5.0');
  assert.match(httpSource, /SDK_VERSION\s*=\s*["']6\.5\.0["']/);
});

test('6.5.0 keeps image claims optional in CLI and SDK', () => {
...
test('6.5.0 keeps multi result rendering and request in progress recovery', () => {
...
test('6.5.0 preserves existing media, progress, and source preference documentation', () => {
