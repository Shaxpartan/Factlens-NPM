...
test('1.0.11 release metadata and runtime SDK header stay synchronized', () => {
  assert.equal(packageJson.version, '1.0.11');
  assert.equal(packageLock.version, '1.0.11');
  assert.equal(packageLock.packages?.['']?.version, '1.0.11');
  assert.match(httpSource, /SDK_VERSION\s*=\s*["']1\.0\.11["']/);
});

test('1.0.11 keeps multi result CLI rendering and long request reconnect behavior', () => {
